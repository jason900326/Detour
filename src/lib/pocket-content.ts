...
export type DiscoveryOutcome = Discovery & {
  seconds: number;
};
...
export function discoveryWeight(
  discovery: Discovery,
  context: DiscoverySelectionContext | Environment | undefined,
  previousKind?: DiscoveryKind,
) {
  const resolved = contextValue(context);
  const environments = effectiveEnvironments(discovery);
  return (
    (resolved.environment && environments.includes(resolved.environment)
      ? 1.35
      : 1) * (previousKind === discovery.kind ? 0.8 : 1)
  );
}

export function chooseDiscoveryDifficulty(
  found: DiscoveryOutcome[],
  random = Math.random,
  light = false,
): Difficulty {
...
function weightedPick(
  pool: Discovery[],
  context: DiscoverySelectionContext | Environment | undefined,
  previousKind: DiscoveryKind | undefined,
  random: () => number,
) {
  const weights = pool.map((discovery) =>
    discoveryWeight(discovery, context, previousKind),
  );
  let roll = random() * weights.reduce((sum, value) => sum + value, 0);
  return (
    pool.find((_, index) => (roll -= weights[index]) <= 0) ??
    pool[pool.length - 1]
  );
}
...
export function selectDiscovery(
  ...
  const context = contextValue(rawContext);
  const experienceId = context.experienceId ?? "core";
  const difficulty = chooseDiscoveryDifficulty(found, random, light);
  const available = filterDiscoveries(discoveries, context);
  const unseen = available.filter((item) => !seen.includes(item.id));

  let pool = unseen.filter((item) => item.difficulty === difficulty);

  if (experienceId !== "core") {
    const themed = pool.filter((item) =>
      discoveryMatchesExperience(item, experienceId),
    );
  ...
  if (!pool.length) {
    // The catalogue should always retain universal core fallbacks. Keeping
    // this final escape hatch makes future small packs safe to curate.
    pool = filterDiscoveries(discoveries, {
      ...context,
      experienceId: "core",
  ...
  return weightedPick(pool, context, found.at(-1)?.kind, random);
}
