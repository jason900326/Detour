import { StyleSheet } from 'react-native';
import { INK, MUTED, SIGNAL, SOFT } from '../../theme/detour-theme';

export const recapOverrides = StyleSheet.create({
  v45FinishScroll: {
    paddingHorizontal: 24,
    paddingTop: 14,
    paddingBottom: 104,
  },
  v45FinishTitleWrap: {
    alignSelf: 'center',
    alignItems: 'center',
    position: 'relative',
    marginBottom: 28,
  },
  v45FinishTitleUnderline: {
    position: 'absolute',
    right: 0,
    bottom: -8,
  },
  v45FinishPrimary: {
    height: 64,
    marginTop: 26,
    backgroundColor: SIGNAL,
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    gap: 14,
    borderRadius: 4,
  },
  v45FinishPrimaryArrow: {
    fontSize: 28,
    color: INK,
  },
  v45FinishPrimaryText: {
    fontSize: 22,
    fontWeight: '900',
    color: INK,
  },
  v45FinishSecondary: {
    height: 50,
    marginTop: 10,
    borderWidth: 0,
    alignItems: 'center',
    justifyContent: 'center',
    backgroundColor: 'transparent',
  },
  v45FinishSecondaryText: {
    fontSize: 17,
    fontWeight: '800',
    color: MUTED,
  },

  v52CompleteStory: {
    width: '100%',
    marginBottom: 6,
  },
  v52ReviewStory: {
    width: '100%',
  },
  v52HeroStage: {
    width: '100%',
    aspectRatio: 4 / 5,
    borderRadius: 22,
    overflow: 'hidden',
    backgroundColor: '#E4DED4',
  },
  v52HeroPhoto: {
    width: '100%',
    height: '100%',
    backgroundColor: '#E4DED4',
  },
  v52ThumbRow: {
    marginTop: 10,
    flexDirection: 'row',
    gap: 7,
  },
  v52ThumbPress: {
    flex: 1,
    height: 56,
    borderRadius: 9,
    overflow: 'hidden',
  },
  v52Thumb: {
    width: '100%',
    height: '100%',
    borderRadius: 9,
    borderWidth: 1,
    borderColor: 'transparent',
    backgroundColor: SOFT,
    opacity: 0.68,
  },
  v52ThumbActive: {
    borderColor: SIGNAL,
    opacity: 1,
  },
  v52CompleteCopy: {
    paddingTop: 20,
    paddingBottom: 4,
  },
  v52ReviewCopy: {
    paddingTop: 20,
  },
  v52Eyebrow: {
    fontSize: 14,
    lineHeight: 19,
    fontWeight: '900',
    color: SIGNAL,
  },
  v52Destination: {
    marginTop: 5,
    fontSize: 32,
    lineHeight: 38,
    fontWeight: '900',
    letterSpacing: -1.1,
    color: INK,
  },
  v52TripFacts: {
    marginTop: 12,
    flexDirection: 'row',
    alignItems: 'center',
    flexWrap: 'wrap',
    gap: 8,
  },
  v52TripFact: {
    fontSize: 14,
    lineHeight: 19,
    fontWeight: '800',
    color: MUTED,
  },
  v52TripFactDot: {
    width: 3,
    height: 3,
    borderRadius: 2,
    backgroundColor: '#A49E94',
  },

  v52RouteSection: {
    marginTop: 30,
    paddingTop: 24,
    borderTopWidth: 1,
    borderTopColor: '#D8D1C5',
  },
  v52SectionHeader: {
    flexDirection: 'row',
    alignItems: 'baseline',
    justifyContent: 'space-between',
    marginBottom: 13,
  },
  v52SectionTitle: {
    fontSize: 20,
    lineHeight: 26,
    fontWeight: '900',
    color: INK,
  },
  v52SectionMeta: {
    fontSize: 13,
    fontWeight: '800',
    color: MUTED,
  },
  v52RouteStage: {
    width: '100%',
    height: 190,
    borderRadius: 18,
    overflow: 'hidden',
    backgroundColor: '#EAE4D9',
  },
  v52RouteTrace: {
    width: '100%',
    height: '100%',
    position: 'relative',
    backgroundColor: '#EAE4D9',
    overflow: 'hidden',
  },
  v52RouteTraceLabel: {
    position: 'absolute',
    left: 14,
    bottom: 13,
    paddingHorizontal: 10,
    paddingVertical: 6,
    borderRadius: 999,
    backgroundColor: 'rgba(245,241,232,0.92)',
  },
  v52RouteTraceLabelText: {
    fontSize: 12,
    fontWeight: '900',
    color: INK,
  },
  v52RouteMissing: {
    flex: 1,
    width: '100%',
    alignItems: 'center',
    justifyContent: 'center',
    paddingHorizontal: 34,
    backgroundColor: '#EAE4D9',
  },
  v52RouteMissingTitle: {
    fontSize: 20,
    lineHeight: 26,
    fontWeight: '900',
    textAlign: 'center',
    color: INK,
  },
  v52RouteMissingBody: {
    marginTop: 7,
    fontSize: 14,
    lineHeight: 20,
    fontWeight: '700',
    textAlign: 'center',
    color: MUTED,
  },

  v53CompleteArtifactWrap: {
    width: '100%',
    position: 'relative',
    paddingBottom: 9,
    marginBottom: 4,
  },
  v53CompleteOffsetCard: {
    position: 'absolute',
    left: 9,
    right: -4,
    top: 10,
    bottom: 0,
    borderRadius: 5,
    backgroundColor: SIGNAL,
    transform: [{ rotate: '1.2deg' }],
  },
  v53CompleteArtifact: {
    width: '100%',
    borderRadius: 5,
    overflow: 'hidden',
    backgroundColor: '#FBF7EE',
    borderWidth: 1,
    borderColor: '#CFC6B9',
    shadowColor: '#000',
    shadowOpacity: 0.13,
    shadowRadius: 14,
    shadowOffset: { width: 0, height: 8 },
    elevation: 5,
  },
  v53ArtifactRail: {
    minHeight: 48,
    paddingHorizontal: 16,
    flexDirection: 'row',
    alignItems: 'center',
    backgroundColor: INK,
  },
  v53ArtifactRailBrand: {
    flex: 1.1,
    fontSize: 15,
    fontWeight: '900',
    letterSpacing: 1.5,
    color: '#F5F1E8',
  },
  v53ArtifactRailLabel: {
    flex: 1,
    fontSize: 13,
    fontWeight: '900',
    textAlign: 'center',
    color: SIGNAL,
  },
  v53ArtifactRailDate: {
    flex: 1.2,
    fontSize: 12,
    fontWeight: '800',
    textAlign: 'right',
    color: '#BDB7AD',
  },
  v53HeroStage: {
    width: 'auto',
    aspectRatio: 4 / 5,
    marginHorizontal: 14,
    marginTop: 14,
    borderRadius: 10,
    overflow: 'hidden',
    backgroundColor: '#171613',
    position: 'relative',
  },
  v53HeroStageRoute: {
    backgroundColor: '#EAE4D9',
  },
  v53HeroPhoto: {
    width: '100%',
    height: '100%',
    backgroundColor: '#171613',
  },
  v53CompleteStamp: {
    position: 'absolute',
    right: 11,
    top: 11,
    width: 65,
    height: 65,
    borderRadius: 33,
    borderWidth: 3,
    borderColor: SIGNAL,
    backgroundColor: 'rgba(245,241,232,0.88)',
    alignItems: 'center',
    justifyContent: 'center',
    transform: [{ rotate: '-10deg' }],
  },
  v53CompleteStampTop: {
    fontSize: 10,
    lineHeight: 13,
    fontWeight: '900',
    letterSpacing: 1.1,
    color: SIGNAL,
  },
  v53CompleteStampBottom: {
    marginTop: -1,
    fontSize: 23,
    lineHeight: 25,
    fontWeight: '900',
    color: INK,
  },
  v53FilmStrip: {
    minHeight: 72,
    marginHorizontal: 14,
    marginTop: 9,
    paddingHorizontal: 8,
    paddingVertical: 7,
    borderRadius: 9,
    backgroundColor: INK,
    flexDirection: 'row',
    alignItems: 'center',
    gap: 6,
  },
  v53FilmHole: {
    width: 5,
    height: 38,
    borderRadius: 2,
    backgroundColor: '#5B5750',
  },
  v53FilmPhotos: {
    flex: 1,
    height: 56,
    flexDirection: 'row',
    gap: 5,
  },
  v53FilmPress: {
    flex: 1,
    height: '100%',
    borderRadius: 5,
    overflow: 'hidden',
  },
  v53FilmPhoto: {
    width: '100%',
    height: '100%',
    borderRadius: 5,
    borderWidth: 1.5,
    borderColor: '#4A4741',
    opacity: 0.64,
    backgroundColor: '#2B2925',
  },
  v53FilmPhotoActive: {
    borderColor: SIGNAL,
    opacity: 1,
  },
  v53CompleteCopy: {
    paddingHorizontal: 18,
    paddingTop: 20,
    paddingBottom: 16,
  },
  v53MoodTicket: {
    alignSelf: 'flex-start',
    paddingHorizontal: 10,
    paddingVertical: 5,
    borderRadius: 3,
    backgroundColor: SIGNAL,
    fontSize: 13,
    lineHeight: 17,
    fontWeight: '900',
    color: INK,
  },
  v53Destination: {
    marginTop: 10,
    fontSize: 32,
    lineHeight: 37,
    fontWeight: '900',
    letterSpacing: -1.1,
    color: INK,
  },
  v53TripFacts: {
    marginTop: 15,
    minHeight: 45,
    borderRadius: 4,
    overflow: 'hidden',
    backgroundColor: INK,
    flexDirection: 'row',
    alignItems: 'stretch',
  },
  v53TripFactCell: {
    flex: 1,
    minHeight: 45,
    alignItems: 'center',
    justifyContent: 'center',
    position: 'relative',
  },
  v53TripFactDivider: {
    position: 'absolute',
    left: 0,
    top: 9,
    bottom: 9,
    width: 1,
    backgroundColor: SIGNAL,
    opacity: 0.75,
  },
  v53TripFactText: {
    fontSize: 13,
    lineHeight: 17,
    fontWeight: '900',
    color: '#F5F1E8',
  },
  v53CompleteRouteMark: {
    width: '100%',
    height: 70,
    marginTop: 16,
    opacity: 0.88,
  },

  v53ReviewArtifact: {
    width: '100%',
    overflow: 'hidden',
    borderRadius: 5,
    borderWidth: 1,
    borderColor: '#CFC6B9',
    backgroundColor: '#FBF7EE',
    shadowColor: '#000',
    shadowOpacity: 0.08,
    shadowRadius: 12,
    shadowOffset: { width: 0, height: 7 },
    elevation: 3,
  },
  v53ArchiveRail: {
    minHeight: 48,
    paddingHorizontal: 16,
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    backgroundColor: INK,
  },
  v53ArchiveRailText: {
    fontSize: 14,
    fontWeight: '900',
    letterSpacing: 1.2,
    color: '#F5F1E8',
  },
  v53ArchiveRailMeta: {
    fontSize: 13,
    fontWeight: '900',
    letterSpacing: 1,
    color: SIGNAL,
  },
  v53ArchiveDateTag: {
    position: 'absolute',
    right: 9,
    bottom: 9,
    paddingHorizontal: 9,
    paddingVertical: 5,
    borderRadius: 3,
    backgroundColor: SIGNAL,
  },
  v53ArchiveDateText: {
    fontSize: 11,
    fontWeight: '900',
    color: INK,
  },
  v53ReviewCopy: {
    paddingHorizontal: 18,
    paddingTop: 20,
    paddingBottom: 18,
  },
  v53RouteSection: {
    marginHorizontal: 14,
    marginBottom: 16,
    padding: 13,
    borderWidth: 1,
    borderStyle: 'dashed',
    borderColor: '#BEB5A8',
    backgroundColor: '#F5F1E8',
  },
  v53RouteSectionTop: {
    minHeight: 48,
    marginBottom: 9,
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
  },
  v53RouteSectionKicker: {
    fontSize: 10,
    lineHeight: 14,
    fontWeight: '900',
    letterSpacing: 1,
    color: SIGNAL,
  },
  v53RouteSectionTitle: {
    marginTop: 2,
    fontSize: 19,
    lineHeight: 24,
    fontWeight: '900',
    color: INK,
  },
  v53RouteSectionFlag: {
    width: 21,
    height: 15,
    backgroundColor: SIGNAL,
    transform: [{ rotate: '5deg' }],
  },
  v53RouteStage: {
    width: '100%',
    height: 164,
    borderRadius: 8,
    overflow: 'hidden',
    backgroundColor: '#EAE4D9',
  },

  v45DetailScreen: {
    flex: 1,
    backgroundColor: '#F5F1E8',
    paddingTop: 58,
    overflow: 'hidden',
  },
  v45DetailTop: {
    height: 64,
    paddingHorizontal: 22,
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
  },
  v45DetailTitle: {
    fontSize: 34,
    lineHeight: 40,
    fontWeight: '900',
    letterSpacing: -1.3,
    color: INK,
  },
  v45DetailScroll: {
    paddingHorizontal: 24,
    paddingTop: 10,
    paddingBottom: 76,
  },
  v45DetailThumbRow: {
    display: 'none',
  },
  v45RouteStrip: {
    display: 'none',
  },
  v45NoteCard: {
    marginTop: 24,
    paddingLeft: 16,
    paddingRight: 2,
    paddingVertical: 2,
    borderWidth: 0,
    borderLeftWidth: 4,
    borderLeftColor: SIGNAL,
    backgroundColor: 'transparent',
  },
  v45NoteTitle: {
    fontSize: 14,
    lineHeight: 20,
    fontWeight: '900',
    letterSpacing: 0.5,
    color: SIGNAL,
  },
  v45NoteBody: {
    marginTop: 7,
    fontSize: 17,
    lineHeight: 27,
    fontWeight: '700',
    color: INK,
  },
  v45ShareButton: {
    height: 66,
    marginTop: 28,
    backgroundColor: SIGNAL,
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    gap: 12,
    borderRadius: 4,
  },
  v45ShareIcon: {
    fontSize: 27,
    color: INK,
  },
  v45ShareText: {
    fontSize: 21,
    fontWeight: '900',
    color: INK,
  },

  v45PosterPhoto: {
    width: '100%',
    height: '100%',
    backgroundColor: '#E8E2D7',
  },
  v51PosterPhotoStage: {
    width: '100%',
    height: 284,
    borderRadius: 18,
    overflow: 'hidden',
    backgroundColor: '#171613',
  },
  v51PosterRouteStage: {
    width: '100%',
    height: 284,
    borderRadius: 18,
    overflow: 'hidden',
    backgroundColor: '#EAE4D9',
  },
  v51PosterHomeRoute: {
    width: '106%',
    height: 94,
    marginLeft: '-3%',
    marginTop: 3,
  },

  v45Poster: {
    width: 360,
    height: 640,
    backgroundColor: '#F5F1E8',
    padding: 22,
  },
  v45PosterTop: {
    height: 48,
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
  },
  v45PosterBrand: {
    fontSize: 26,
    fontWeight: '900',
    letterSpacing: -1.2,
    color: INK,
  },
  v45PosterDate: {
    fontSize: 13,
    fontWeight: '800',
    color: MUTED,
  },
  v45PosterCopy: {
    paddingTop: 15,
  },
  v45PosterMood: {
    fontSize: 34,
    lineHeight: 38,
    fontWeight: '900',
    letterSpacing: -1.3,
    color: INK,
  },
  v45PosterDestination: {
    marginTop: 3,
    fontSize: 20,
    lineHeight: 25,
    fontWeight: '900',
    color: SIGNAL,
  },
  v45PosterOrangeRule: {
    display: 'none',
  },
  v45PosterFacts: {
    marginTop: 10,
    flexDirection: 'row',
    gap: 18,
  },
  v45PosterFact: {
    fontSize: 14,
    fontWeight: '800',
    color: MUTED,
  },

  v51CompleteCard: {
    display: 'none',
  },
  v51ReviewHeroStage: {
    display: 'none',
  },
  v46ReviewArtwork: {
    display: 'none',
  },
});
