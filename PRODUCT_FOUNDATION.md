...
> 文件權責：本文件是跨主題產品基線；日期較晚且明確標示為 ACTIVE／已定案的主題文件，可只覆蓋其負責的主題。完整閱讀順序見 [`docs/README.md`](./docs/README.md)。


## 1. 一句話定位

**Detour 把現實世界變成一場 10–60 分鐘的小型探索遊戲。**
...
時間固定為 10–60 分鐘，每 5 分鐘一個節點：

- 10、15、20、25、30、35、40、45、50、55、60 分鐘。
- 不提供 5 分鐘以下或超過 60 分鐘的旅程。
- 滑動最後吸附到每 5 分鐘一個節點。
...
因此不能只用公尺數把 10–60（每 5 分鐘一個節點） 線性放大。

特別是 **50–60 分鐘**，如果只是把終點拉遠、任務塞更多，Scene 品質與體驗都容易下降。
...
目前正式只保留三個 Mood；版面是「隨便走」全寬主卡，下方並排「吃東西」與「色色的」。

| Mood | 目的地與玩法 |
| --- | --- |
| **隨便走** | Detour Scene Pool；融合原「這是哪／怪怪的」精神，去找原本不會遇到的城市發現 |
| **吃東西** | Food Pool；最後得到低承諾的食物或飲料獎勵 |
| **色色的** | 與隨便走共用 Detour Scene Pool；整趟只追一個隨機顏色 |

規則：
- Mood 名稱要短、口語、有一點好奇感，不要像設定分類。
- Mood 可以影響 Scene、路線氣質或玩法，但不要把尋找變得主觀複雜。
- 不為了補齊版面增加意義不足的第四個 Mood。
...
目標依旅程進度、有效移動與導航情境出現，約每 5 分鐘一個節奏，但不使用固定計時器硬切。這組數量是目前外測版本的正式規範。
...
1. **時間系統**：時間滑桿固定 10–60 分鐘、每 5 分鐘一個節點；Engine 使用整趟時間 budget，選擇時間是規劃參考，不是截止時間。
2. **Mood 系統**：固定成「隨便走 / 吃東西 / 色色的」三個選項；隨便走全寬，另外兩張並排，不補空白第四格。
...
8. **50–60 分鐘結構實驗**：不要直接拉長單一路線；測試 checkpoint / 兩段式 / 弧線或不完整環狀路線。
...
- 一般 Mood 的目標數量與出現節奏是否需要在外測後微調。
- 一個尋找應維持多久 / 多遠才自然換下一個。
- 第一個尋找最佳出現時間是 30 秒、1 分鐘還是 2–3 分鐘。
- 50–60 分鐘是固定兩段式，還是依候選品質動態決定段數。
...
- **照片數量不再綁定舊版六張上限**：目標數量依 10–60 分鐘規範配發，拍照容量需足以支援該趟目標。
- **長旅程不是更多 checklist**：10–15 / 20–25 / 30–35 / 40–45 / 50–60 分鐘的普通尋找數量為 3 / 5 / 7 / 9 / 10；長時間的價值來自路線、區域與終點。
